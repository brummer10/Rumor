...
void plugin_set_window_size(int *w,int *h,const char * plugin_uri, float scale) {
    (*w) = 451 * scale; //set initial width of main window
    (*h) = 234 * scale; //set initial height of main window
}
...
void plugin_create_controller_widgets(X11_UI *ui, const char * plugin_uri, float scale) {
    ...
    widget_get_scaled_png(ui->win, LDVAR(background_150x1_png));
    ui->widget[0] = add_lv2_image_toggle (ui->widget[0], ui->win, 2, "Bypass", ui, 60,  60, 87 * scale, 116 * scale);
    widget_get_png(ui->widget[0], LDVAR(switch_82x2_png));

    ui->widget[1] = add_lv2_knob (ui->widget[1], ui->win, 3, "Intensity", ui, 180,  60, 86 * scale, 117 * scale);
    widget_get_png(ui->widget[1], LDVAR(knob_120x101_png));
    set_adjustment(ui->widget[1]->adj, 0.000, 0.000, 0.000, 1.000, 0.010, CL_CONTINUOS);

    ui->widget[2] = add_lv2_knob (ui->widget[2], ui->win, 4, "Volume", ui, 300,  60, 86 * scale, 117 * scale);
    widget_get_png(ui->widget[2], LDVAR(knob_120x101_png));
